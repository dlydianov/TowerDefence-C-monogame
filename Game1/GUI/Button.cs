...
namespace Game1.GUI
{
    public  class Button :Sprite
    {
        ...
        private ButtonsStatus state = ButtonsStatus.Normal;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (isMouseOver && state != ButtonsStatus.Pressed)
            {
                state = ButtonsStatus.MouseOver;
            }
            else if (isMouseOver == false && state != ButtonsStatus.Pressed)
            {
                state = ButtonsStatus.Normal;
            }
                ...
                previousState.LeftButton == ButtonState.Pressed)
            {
                ...
                else if (state == ButtonsStatus.Pressed)
                {
                    state = ButtonsStatus.Normal;
                }
            }
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            switch (state)
            {
                case ButtonsStatus.Normal:
                    spriteBatch.Draw(texture, bounds, Color.White);
                    break;
                case ButtonsStatus.MouseOver:
                    spriteBatch.Draw(hoverTexture, bounds, Color.White);
                    break;
                case ButtonsStatus.Pressed:
